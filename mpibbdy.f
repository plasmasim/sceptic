...
c myid returns the process id in the sor_comm (OUT)
      integer myid
c Am I on the outer part of the domain (Need BC for sorrelax ?)
      logical out
c End of arguments.
c---------------------------------------------------------------
...
c Initialize
         lflag=.true.
         call MPI_COMM_RANK(sor_comm, myid, ierr )
         call MPI_COMM_SIZE(sor_comm, numprocs, ierr )
...
c Create topology
         lreorder=.true.
         call MPI_CART_CREATE(sor_comm,ndims,irdims,lrperiod,
