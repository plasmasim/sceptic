...
c     If we are on the bodrer or not, for the BC
      logical out
...
c         if (mod(isor_k+2,3).eq.0) then
     ...
     $        icoords,iLcoords,myside,myorig,myorig1,myorig2,
     $        icommcart,mycartid,myid,lflag,out)
c         endif
...
c     Test convergence
         if (mod(isor_k+2,30).eq.0) then
            call testifconverged(sor_eps,delta,
...
c converged, but the total spread depends on multiple blocks.
      subroutine testifconverged(eps,delta,lconverged,
     $     icommcart)
      include 'mpif.h'
      logical lconverged
      real delta
...
c doing it in place.
      ...
      call MPI_ALLREDUCE(MPI_IN_PLACE,delta,1,MPI_REAL,
...
c***********************************************************************
      subroutine fcalc_sor(Li,Lj,ni,nj,jac,mi,eps,k,sor_comm)
      ...
      real sor_jac,sor_eps,sor_del,jac,eps
...
c testing arrays
      ...
      call sor2dmpi(sor_comm,Li,Lj,ni,nj,debyelen,bcphi,
     $     phi(1,0),rho(1,0),ictl,ierr,myid,idim1,idim2,
     $     apc(1),bpc(1),cpc(1,0),dpc(1,0),fpc(1,0),gpc(0,1))
