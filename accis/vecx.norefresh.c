...
/* ********************************************************************** */
/*
Currently this does no redrawing. Clearly that is much harder. 1 Dec 96.
*/
...
{  
  ...
  if(second == 0){
    ...
    *vmode=88;
    *ncolor=15;
    /* Set up a default size. This is overruled by Accis*geometry resources */
    n = 0;
    ...
    initDefaultColors();
    /* This seems only to get events on a one cycle delayed basis.
    XtAddEventHandler(accis_drawing,StructureNotifyMask,FALSE,
		      config_handler,&s_s);
    */
...
/* ******************************************************************** */
/* End plotting and return to text editing. Not needed perhaps. */
/* Subroutine */ int txtmode_()
{
  XFlush(accis_display);
  getchar();
} /* txtmode_ */
