...
{  
  ...
  *scrypix=s_s.height;
  ...
  /* This is quite problematic. Giving intermittent BadMatch errors
     It does not seem generally to be necessary. Window manager should
     probably handle the focus setting, but it would be better if we could
     guarantee to put the focis in the drawing window.*/
  XRaiseWindow(accis_display, accis_window); /* Prevent Bad Match? */
