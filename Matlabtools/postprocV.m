function postprocV(folder)

% postprocX(folder)
% Outputs diagnostics as a function of v_d for the files included in
% the file "titles" in the folder "folder"
    ...
    clear V L K Vd Fc Fo Ffield Felec Fion Ftot Felec2 Fion2 Ffield2 Ftot2 fluxtot;
        figure;
        hold all
    ...
    % sort
    BF=[Vd',fluxtot',K',V',Ffield',Felec',Fion',Ftot',Ffield2',Felec2',Fion2',Ftot2'];
    BF=sortrows(BF,1);

    figure(1)
    plot(BF(:,1),BF(:,2))
    title('Ion Current','FontSize',18)
    xlabel('v_d','FontSize',18)
    ylabel('I_i/ (4 \pi r_p^2 v_{ti}/ 2\pi^{1/2})','FontSize',18)

    figure(2)
    plot(BF(:,1),BF(:,5),'b^-',BF(:,1),BF(:,6),'k-',BF(:,1),BF(:,7),'og-',BF(:,1),BF(:,8),'rs-')
    ...
    figure(3)
    semilogx(BF(:,1),BF(:,9),'b^-',BF(:,1),BF(:,10),'k-',BF(:,1),BF(:,11),'og-',BF(:,1),BF(:,12),'+m-',BF(:,1),BF(:,13),'rs-')
